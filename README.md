...
이 프로젝트는 강화 학습 알고리즘인 DQN을 CarRacing-v2에서 학습시키고, 그 결과를 시각화한 내용입니다.
...
- **DQN-CarRacing.ipynb**:  
  DQN 알고리즘을 통해 CarRacing-v2 환경에서 자동차를 주행하는 에이전트를 학습시키고, 학습 성과를 평가하는 코드입니다.
  
## src

- **CNN.py**:  
  DQN에서 상태 공간을 처리하기 위한 CNN 모델 정의 & 입력 이미지를 통해 각 action의 value 출력합니다. 그리고 kernel & stride를 바꾸어 학습시켰습니다.
...
- **evaluate.py**:  
  CarRacing-v2 환경에서 DQN 에이전트를 평가하는 기능 정의합니다.
